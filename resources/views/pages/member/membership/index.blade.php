                    ...
                    <div class="col-4">
                       <p style="color: black;font-size:17px;"> Active Plan: <span style="font-weight: bold">
                        @foreach ($memberships as $membership)
                            @if ($membership->payment_type == 0)
                                Starter
                            @elseif ($membership->payment_type == 1)
                                Basic
                            @else
                                Professional
                            @endif
                     ...
                     <div class="col-4">
                        <p style="color: black;font-size:17px;"> Date End: <span style="font-weight: bold">
                            ...
                            <td class="align-middle">
                                @foreach ($memberships as $membership)
                                @if ( $membership->payment_type == 0 )
                                    {{ date('M-d-Y', strtotime("+1 months",strtotime($membership->created_at))) }}
                                @elseif ( $membership->payment_type == 1)
                                    {{ date('M-d-Y', strtotime("+3 months",strtotime($membership->created_at))) }}
                                @else
                                   {{  date('M-d-Y', strtotime("+12 months",strtotime($membership->created_at))) }}
                                @endif
                                @endforeach
                            </td>
                        </span></p>
...
<div class="snip1404">
      ...
      <div class="plan-select"><a href="">
        ...
        {{-- @foreach ($memberships as $membership)
                ...
                @else
                    ...
                    @else
                        <form action="{{ url('charge') }}" method="post">
                            <input style="display:none" type="number" name="amount" value="200"/>
                            <input style="display:none" type="number" name="payment_type" value="0"/>
                            {{ csrf_field() }}
                            <input type="submit" style=" all: unset;
                            cursor: pointer;" name="submit" class="plan-select" style="display:inline;" value="Select Plan">
    ...
    @endif
        ...
        @else
                @if ($memberships)
                <form action="{{ url('charge') }}" method="post">
                    <input style="display:none" type="number" name="amount" value="500"/>
                    <input style="display:none" type="number" name="payment_type" value="1"/>
                    {{ csrf_field() }}
                    <input type="submit" style=" all: unset;
                    cursor: pointer;" name="submit" class="plan-select" style="display:inline;" value="Select Plan" >
    ...
    @else
        @if ($memberships) --}}
            {{-- <form action="{{ url('charge') }}" method="post">
                <input style="display:none" type="number" name="amount" value="1500"/>
                <input style="display:none" type="number" name="payment_type" value="2"/>
                {{ csrf_field() }}
                <input type="submit" style=" all: unset;
                cursor: pointer;" name="submit" class="plan-select" style="display:inline;" value="Select Plan" >
